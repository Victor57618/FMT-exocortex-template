...
# === Windows locale: auto UTF-8 -> CP1251 for Kimi stdin (WP-25) ===
# Detect Windows-class shell (MINGW/MSYS/CYGWIN) where Kimi runs in CP1251 charmap.
# On macOS/Linux this block is a no-op; KIMI_STDOUT_ENCODING stays utf-8.
...
with open(sys.argv[1], 'r', encoding='utf-8') as f:
    txt = f.read()
# Replace common non-CP1251 symbols with ASCII analogs before lossy encode
replacements = {
    '→': '->',   # right arrow
    '←': '<-',   # left arrow
    '↔': '<->',  # left-right arrow
    ...
    '✓': 'OK',   # check mark
    '✗': 'X',    # ballot X
    # Note: em-dash (—) and en-dash (–) are valid in CP1251 (0x97/0x96), keep them as-is
}
for src, dst in replacements.items():
    txt = txt.replace(src, dst)
# Encode with CP1251, replace any remaining surprises with '?'
with open(sys.argv[2], 'wb') as f:
    f.write(txt.encode('cp1251', errors='replace'))
PYEOF
    PROMPT_FILE="$PROMPT_FILE_CP1251"
    KIMI_STDOUT_ENCODING="cp1251"
    ;;
esac
...
# WP-25: capture stdout to file (bytes), then decode with platform encoding and
# filter "To resume" line on CLEAN TEXT, not on raw bytes (grep -v on CP1251
# stdout sees NUL bytes from non-ASCII and treats output as binary on Windows).
KIMI_RAW="$TMP_ROOT/kimi-raw.bytes"
if [ "${IWE_PEER_INLINE:-0}" = "1" ]; then
  # Inline mode: prompt содержит файлы, --add-dir не нужен
  perl -e 'alarm 300; exec @ARGV' -- "$KIMI_BIN" --quiet --yolo \
    ${MODEL_ARG[@]+"${MODEL_ARG[@]}"} \
    < "$PROMPT_FILE" \
    > "$KIMI_RAW" 2>/dev/null
else
...
# Decode + filter on clean text, emit UTF-8 bytes (WP-25)
# Use sys.stdout.buffer to bypass Python's locale-dependent stdout encoder
# (on Windows MSYS, default sys.stdout is CP1251, which would re-corrupt output).
KIMI_OUTPUT=$(python3 - "$KIMI_RAW" "${KIMI_STDOUT_ENCODING:-utf-8}" <<'PYEOF'
import sys
with open(sys.argv[1], 'rb') as f:
    data = f.read()
enc = sys.argv[2]
text = data.decode(enc, errors='replace')
lines = [ln for ln in text.splitlines() if not ln.startswith('To resume this session:')]

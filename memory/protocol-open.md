...
> **Вчерашний WakaTime (неблокирующее обслуживание):** если записи `day_close` за вчера нет в Neon, поставить фоновое восстановление через WakaTime API и продолжить Day Open. Успех, недоступность API или ошибка записи не задерживают формирование плана; результат сообщается в «Требует внимания» только когда меняет доступную пилоту метрику или требует действия. Причина: ретроспективная метрика не определяет открытие текущего дня.

...
for f in "${NEW_FILES[@]}" "${UPDATED_FILES[@]}"; do
    ...
    if grep -q '/c/Users/tocpeople/IWE' "$filepath" 2>/dev/null; then
        if [ -n "$DETECTED_WORKSPACE" ]; then
            sed_inplace "s|/c/Users/tocpeople/IWE|$DETECTED_WORKSPACE|g" "$filepath"
            PLACEHOLDER_HIT=$((PLACEHOLDER_HIT + 1))
        fi
    fi
    if grep -q '/c/Users/tocpeople' "$filepath" 2>/dev/null; then
        sed_inplace "s|/c/Users/tocpeople|$HOME|g" "$filepath"
        PLACEHOLDER_HIT=$((PLACEHOLDER_HIT + 1))
    fi
done

...
| **Сессия** | Закрытие (Quick Close) | `/run-protocol close` → `protocol-close.md § Сессия` | «закрываю» / «всё» / «закрывай» |
| **День** | Открытие | `protocol-open.md § День` | «открывай» / «открывай день» |
| **День** | Работа | `protocol-work.md § День` | Между Day Open и Day Close |
| **День** | Закрытие (Day Close) | `/run-protocol day-close` → `protocol-close.md § День` | «закрываю день» / «итоги дня» |
| **Неделя** | Закрытие (Week Close) | `/run-protocol week-close` → `protocol-close.md § Неделя` + `week-review.md` | Пн 00:00 / «итоги недели» |
...
|------------|------|-------|-----------|
| Правило для всех репо (1-3 строки) | `/c/Users/tocpeople/IWE/CLAUDE.md` | Сразу | Нет |
...
> **Рабочая директория:** Все сессии Claude Code открывать из `/c/Users/tocpeople/IWE/` (корень workspace). НЕ из sub-директорий (`DS-strategy/`, `DS-IT-systems/` и т.д.) — иначе `memory/` не резолвится. `/c/Users/tocpeople/IWE/memory/` — симлинк на auto-memory Claude Code.
...
- **Done РП**: зачёркнуть всю строку обычным шрифтом, без bold (`| ~~#~~ | ~~название~~ | ... |`). **Тильда внутри `~~`:** символ `~` внутри strikethrough ломает рендер GitHub (интерпретируется как конец `~~`). При зачёркивании заменять `~30 мин` → `30 мин`, `/c/Users/tocpeople/IWE/` → `IWE/`.
...
> **Скилл ≠ Роль ≠ Протокол.** Скилл (`.claude/skills/`) = точечное действие с входом/выходом. Роль = контекст на всю сессию (CLAUDE.md репо). Протокол = ритуал стадии ОРЗ (protocol-*.md). Метод (Pack) → скилл (FMT) → рабочий экземпляр (/c/Users/tocpeople/IWE/.claude/skills/). Skill Discovery: `protocol-work.md §2b`.
...
- `/c/Users/tocpeople/IWE/` — рабочая директория (шаблон: `/c/Users/tocpeople/IWE`)

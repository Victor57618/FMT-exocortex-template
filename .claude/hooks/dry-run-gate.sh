...
if [ "$TOOL_NAME" = "Bash" ]; then
    ...
    # Исключение: очистка самого sentinel-флага — легитимный teardown.
    # Штатная команда rm -f /tmp/iwe-dry-run-*.flag иначе попадает под матчер
    # (rm|mv) и самоблокируется (sticky-sentinel). Разрешаем rm/find-delete,
    # только если единственная цель — sentinel-файл dry-run.
    if echo "$CMD" | grep -qE '(rm|find)([[:space:]]|$)' \
       && echo "$CMD" | grep -qE '/tmp/iwe-dry-run-[^[:space:]]*\.flag' \
       && ! echo "$CMD" | grep -qE '[[:space:]](DS-|FMT-|/c/|~|\$HOME)'; then
        exit 0
    fi

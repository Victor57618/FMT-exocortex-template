...
**Экономия контекста (issue #207):** DayPlan не читать целиком. Нужны только три блока: frontmatter (`Read` первых ~10 строк) + секции «Итоги», «Завтра начать с», «Требует внимания» (вытащить `grep`/`awk` по заголовкам или `Read` с offset на конкретные секции). Полный файл — только если структура нестандартная и адресные секции не найдены.
...
Day Open выполняет только **мини-триаж**. В DayPlan попадают:

1. действие, которое должно изменить сегодняшний план;
2. срочный сигнал с угрозой остановки работы;
3. кандидат, по которому сегодня требуется решение пилота, включая передачу знания Экстрактору.

Остальные заметки утром не классифицировать и не переносить в DayPlan. Полная
классификация по PD.FORM.083, пометка и архивация принадлежат Note-Review.
...
**Гиперссылки на вынесенные заметки (БЛОКИРУЮЩЕЕ):** каждая заметка, прошедшая мини-триаж и включённая в DayPlan, — markdown-ссылка на источник (`inbox/fleeting-notes.md` для свежих, `archive/notes/Notes-Archive.md#L<line>` для обработанных, `inbox/captures.md` для знания). Формат строки таблицы: `[«заголовок»](путь#L<line>) (DD мес HH:MM)`.
